    ...
    public class LoginResultUserModel
    {
        ...
        public ClaimsPrincipal UpdateClaimsPrincipal(ClaimsPrincipal claimsPrincipal, string provider)
        {
            ...
            if (!String.IsNullOrWhiteSpace(FirstName))
                claims.Add(new Claim(OpenIddictConstants.Claims.GivenName, FirstName, "string", provider));

            if (!String.IsNullOrWhiteSpace(LastName))
                claims.Add(new Claim(OpenIddictConstants.Claims.FamilyName, LastName, "string", provider));

            if (!String.IsNullOrWhiteSpace(Email))
                claims.Add(new Claim(OpenIddictConstants.Claims.Email, Email, "string", provider));

            if (!String.IsNullOrWhiteSpace(PhoneNumber))
                claims.Add(new Claim(OpenIddictConstants.Claims.PhoneNumber, PhoneNumber, "string", provider));
