...
export class EndpointRulesListComponent {
    ...
    constructor(
        ...
        private uiService: AppUIService) {
            ...
            uiService.Set(ui => ui.Content.Scrollable = true);
    }
    ...
    IsRuleTerminating(rule: EndpointRuleListDto) {

        if(!rule.Actions) {
            return false;
        }
        var found = rule.Actions.findIndex(r => r.Enabled && r.Terminating);
        return found != -1;
    }
    ...
    drop(event: CdkDragDrop<string[]>) {
        ...
        if (event.previousContainer === event.container) {
            const rules = this.endpointRulesQuery.getAll().sort(this.rulesService.SortRules)
            const currentItem = rules[event.previousIndex];
            ...
            if (direction === 'forward') {
                if (event.currentIndex === rules.length - 1) {
                    this.rulesService.UpdateRuleOrder(currentItem, this.rulesService.GetNextLastOrder())
                    //currentItem.Order = this.rulesService.GetNextLastOrder()
                } else {
                    const beforeItem = rules[event.currentIndex]
                    const nextItem = rules[event.currentIndex + 1]
                    this.rulesService.UpdateRuleOrder(currentItem, ((nextItem.Order - beforeItem.Order) / 2) + beforeItem.Order)
                    // currentItem.Order = ((nextItem.Order - beforeItem.Order) / 2) + beforeItem.Order
                }
            } else if (direction === 'back') {
                if (event.currentIndex === 0) {
                    this.rulesService.UpdateRuleOrder(currentItem, rules[0].Order / 2)
                    // currentItem.Order = rules[0].Order / 2
                } else {
                    const beforeItem = rules[event.currentIndex - 1]
                    const nextItem = rules[event.currentIndex]
                    this.rulesService.UpdateRuleOrder(currentItem, ((nextItem.Order - beforeItem.Order) / 2) + beforeItem.Order)
                    // currentItem.Order = ((nextItem.Order - beforeItem.Order) / 2) + beforeItem.Order
                }
            }
