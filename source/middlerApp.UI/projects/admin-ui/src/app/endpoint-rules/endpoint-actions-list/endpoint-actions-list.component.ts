...
export class EndpointActionsListComponent implements OnInit, ControlValueAccessor, OnDestroy {
    ...
    // @Input() ruleId: string;
    private terminateActionIndex = -1;
    private actionsSubject$ = new BehaviorSubject<Array<EndpointAction>>([]);
    actions$ = this.actionsSubject$.asObservable().pipe(
        tap(actions => {
            this.terminateActionIndex = actions.findIndex(act => act.Enabled && act.Terminating)
        })
    );
    ...
    CreateAction(actionType: string, position: string, currentAction?: EndpointAction) {
        ...
        act.Order = this.CalculateOrder(position, currentAction) ?? 0;
    ...
    private getNewActionsId() {
        ...
        const filterInt = function (value:any) {
    ...
    SetActionTerminate(action: EndpointAction, value: boolean) {

        this.actions = this.actions.map(act => {
            if (act == action) {
                act.Terminating = value;
            }
            return act;
        });
        this.propagateChange(this.actions);
    ...
    GetTerminateStatus(endpointAction: EndpointAction, index: number) {

        if (!endpointAction.Enabled)
            return null;

        if (this.IsAfterTerminate(index))
            return null;

        return this.GetTerminateStatusText(endpointAction, index);


    }

    GetTerminateStatusText(endpointAction: EndpointAction, index: number) {

        const isLast = this.IsLastExecuted(index)

        if (!endpointAction.Terminating && isLast)
            return "Continue with next Endpoint Rule";


        if (endpointAction.Terminating && isLast)
            return "Stop processing further Endpoint Rules";

        if (endpointAction.Terminating && !isLast)
            return "Stop processing further Actions / Endpoint Rules";

        return null;

    }

    IsAfterTerminate(index: number) {

        if(this.terminateActionIndex == -1)
            return false;
        return index > this.terminateActionIndex;

    }

    IsLastExecuted(index: number) {

        var enabledActions = this.actions.filter((act, ind) => act.Enabled && ind > index);
